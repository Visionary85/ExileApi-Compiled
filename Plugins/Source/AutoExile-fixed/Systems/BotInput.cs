...
namespace AutoExile.Systems
{
    ...
    public static class BotInput
    {
        ...
        public static bool ClickEntity(GameController gc, Entity entity)
        {
            ...
            // Capture the current cancellation token. When Cancel() is called on area change,
            // the token is cancelled and DoClickEntityWithVerify exits before firing inputs.
            var ct = _clickCts.Token;
            _ = DoClickEntityWithVerify(gc, entity, screenCenter, halfW, halfH, windowRect, settle, hold, ct);
        ...
        private static async Task DoClickEntityWithVerify(
            ...
            SharpDX.RectangleF windowRect, int settleMs, int holdMs, CancellationToken ct)
        {
            for (int attempt = 0; attempt < MaxHoverAttempts; attempt++)
            {
                ...
                try
                {
                    ...
                    if (targetable?.isTargeted == true)
                    {
                        ...
                        if (ct.IsCancellationRequested) { SendLeftUp("entity-verified"); return; }
                        await SendDelay(ct);
            ...
            await Task.Delay(holdMs);
            if (ct.IsCancellationRequested) { SendLeftUp("entity-fallback"); return; }
            await SendDelay(ct);
            ...
            Func<SharpDX.RectangleF?>? rectProvider = null)
        {
            ...
            NextActionAt = DateTime.Now.AddMilliseconds(
                MaxHoverAttempts * (moveMs + settle) + hold + ActionCooldownMs);
            var ct = _clickCts.Token;
            _ = DoClickLabelVerified(entity, rect, windowRect, settle, hold, rectProvider, ct);
            ...
            for (int attempt = 0; attempt < MaxHoverAttempts; attempt++)
            {
                ...
                try
                {
                    ...
                    if (targetable?.isTargeted == true)
                    {
                        ...
                        await Task.Delay(holdMs);
                        if (ct.IsCancellationRequested) { SendLeftUp("label-verified"); return; }
                        await SendDelay(ct);
        ...
        /// </summary>
        private static async Task SendDelay(CancellationToken ct = default)
        {
            if (ct.IsCancellationRequested) return;
        ...
        /// </summary>
        private static async Task MoveCursorTo(Vector2 target, CancellationToken ct = default)
        {
        ...
        // CancellationTokenSource for in-flight fire-and-forget async click tasks.
        // Cancelled and renewed in Cancel() so that async click loops started before
        // an area change (e.g. portal click carryover) do not fire additional inputs
        // into the new area — specifically, preventing exit-portal clicks in a freshly
        // loaded simulacrum.
        private static CancellationTokenSource _clickCts = new();
        ...
        public static void Cancel()
        {
            ...
            // Abort any in-flight DoClickEntityWithVerify / DoClickLabelVerified loops.
            // These are fire-and-forget tasks; the only way to stop them is via a token.
            _clickCts.Cancel();
