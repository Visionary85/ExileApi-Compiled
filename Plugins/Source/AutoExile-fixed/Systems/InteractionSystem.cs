...
namespace AutoExile.Systems
{
    ...
    public class InteractionSystem
    {
        ...
        private InteractionResult TickGroundItem(GameController gc)
        {
            ...
            var labelRect = labelDesc.Label.GetClientRect();
            ...
            Func<SharpDX.RectangleF?> rectProvider = () =>
            {
                ...
                return f && desc?.Label?.IsVisible == true ? desc?.Label.GetClientRect() : null;
            };
        ...
        private Entity? FindOverlappingLabel(GameController gc, long targetEntityId, SharpDX.RectangleF targetRect)
        {
            try
            {
                foreach (var label in gc.IngameState.IngameUi.ItemsOnGroundLabelsVisible)
                {
                    ...
                    if (label.ItemOnGround?.Id == targetEntityId) continue; // skip self

                    var otherRect = label.Label.GetClientRect();
                    ...
                    if (overlaps && label.ItemOnGround != null)
                        return label.ItemOnGround;
                }
            }
            catch { }
        ...
        private (bool Found, LabelOnGround? Label) FindGroundItemLabel(
            GameController gc, long entityId)
        {
            try
            {
                foreach (var label in gc.IngameState.IngameUi.ItemsOnGroundLabelsVisible)
                {
                    if (label.ItemOnGround?.Id == entityId)
                        return (true, label);
                }
            }
            catch { }
